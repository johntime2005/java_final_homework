...
public class DatabaseInitializer {
    ...
    private static void createTables(Connection connection) throws SQLException {
        try (Statement stmt = connection.createStatement()) {
            // 创建用户表
            stmt.execute("IF NOT EXISTS (SELECT * FROM sys.objects " +
                    "WHERE object_id = OBJECT_ID(N'[dbo].[library_user]') AND type in (N'U')) " +
                    "BEGIN " +
            ...
            // 创建借阅记录表
            stmt.execute("IF NOT EXISTS (SELECT * FROM sys.objects " +
                    "WHERE object_id = OBJECT_ID(N'[dbo].[user_book]') AND type in (N'U')) " +
                    "BEGIN " +
    ...
    private static void createAdminUser(Connection connection) throws SQLException {
        try (Statement stmt = connection.createStatement()) {
            ...
            if (rs.next() && rs.getInt(1) == 0) {
                // 创建默认管理员账户
                stmt.execute("INSERT INTO library_user (username, password, user_type, balance) " +
